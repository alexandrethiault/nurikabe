...
blue, beige = (208, 250, 249), (255, 244, 223)
red, purple = (255,0,0), (255, 0, 255)

def lecture_grille(num):
    grille = []
...
def init_data():
    ...
    for name in os.listdir(".\Grilles"):
        if name.endswith(".txt"):
            try:
                grilles[name[:-4]] = lecture_grille(name[:-4])
...
def show_rules():
    pygame.draw.rect(fenetre, white,(0,height-132,width,132))
    ...
    lines = text.split("\n")
    y = height - 20*len(lines)
    for line in lines:
        message_display(line, 20, width//2, y)
        y+=20
    clock.tick(25)
...
def show_help():
    ...
    lines = text.split("\n")
    y = height - 30*len(lines)
    for line in lines:
        message_display(line, 30, width//2, y)
        y+=30
    #clock.tick(25)
...
def verify():
    ...
    for i in range(n):
        for j in range(m):
            ...
            elif grille[i][j].isdigit(): # == 2, terre
                k = int(grille[i][j])
...
def print_grid():
    ...
    fenetre.fill(white)
    n, m, grille = grilles[current_grid]
    sq_len = min(750//max(n,m), square_default)
    xdeb = width//2 - sq_len * m // 2
    ydeb = (height-120)//2 - sq_len * n // 2
    xfin = xdeb + sq_len * m
    yfin = ydeb + sq_len * n
    for i in range(n+1):
        pygame.draw.line(fenetre, (0,0,0), (xdeb,ydeb+sq_len*i), (xfin,ydeb+sq_len*i), 2)
    for i in range(m+1):
        pygame.draw.line(fenetre, (0,0,0), (xdeb+sq_len*i,ydeb), (xdeb+sq_len*i,yfin), 2)
    for i in range(n):
        for j in range(m):
            if states[-1][i*m+j] == 0: color = white
            elif states[-1][i*m+j] == 1: color = blue
            elif states[-1][i*m+j] == 2: color = beige
            elif states[-1][i*m+j] == 3: color = red
            elif states[-1][i*m+j] == 4: color = purple
            pygame.draw.rect(fenetre, color, Rect(xdeb+j*sq_len+2, ydeb+i*sq_len+2, sq_len-2, sq_len-2))
            if grille[i][j].isdigit():
                message_display(grille[i][j], sq_len-2, xdeb+j*sq_len+sq_len//2, ydeb+i*sq_len+sq_len//2, False)
...
def onclick(b, x, y):
    ...
    if b in [2, 4, 5]:
        ...
        if write:
            ...
            pygame.mouse.set_cursor(*cursor_erase)
    elif current_grid is not None and 100 <= x <= width-100:
        n, m, grille = grilles[current_grid]
        sq_len = min(750//max(n,m), square_default)
        xdeb = width//2 - sq_len * m // 2
        ydeb = (height-120)//2 - sq_len * n // 2
        ...
        if xdeb<=x<=xfin and ydeb<=y<=yfin:
            ...
            if b == 1:
                ...
                if write and grilles[current_grid][2][i][j].isdigit():
                    states[-1][i*m+j] = 3
            else: # b==3
...
def show_buttons(from_undo=False):
    xbtn = ybtn = 5
    for i in grilles:
        button(i, xbtn,ybtn, 90,30, gray1 if i!=current_grid else gray3,gray3, lambda:load_grid(i))
        if i in won:
            fenetre.blit(checkmark, (xbtn + 96, ybtn))
        ybtn+=35
...
def main():
    global grilles, screen, fenetre, clock, current_grid, states, write, cursor_write, cursor_erase, checkmark, won
